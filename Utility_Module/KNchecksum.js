...
export async function checkBcodeStructure() {
  ...
  for (const c of structureChecks) {
    ...
    const typeOK = c.type === 'file' ? fs.lstatSync(c.path).isFile() : fs.lstatSync(c.path).isDirectory();
    if (!exists || !typeOK) {
      console.error(`{ERROR} ${c.errorMsg}`);
      if (c.required) allPass = false;
    } else {
      const msg = c.type === 'file' ? analyzeFileHealth(c.path) : '{OK}';
